...
export default function App() {
  ...
  const exportData = (exportType) => {
    ...
    if (exportType === 'staff_defects' && filterStaff !== 'all') {
      ...
      fileName = `${statsMonth}_全體查核彙整.csv`;
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-sm bg-white p-8 rounded-[2rem] shadow-xl text-center border border-gray-200">
            <h2 className="text-xl font-bold mb-6 text-gray-800">身分驗證</h2>
            <div className="space-y-4">
              ...
              <button onClick={() => { if(loginUser === DEFAULT_ADMIN.username && loginPass === DEFAULT_ADMIN.password) { setIsAdmin(true); setView('admin'); } else { alert("驗證失敗"); } }} className="w-full py-4 bg-[#c5a065] text-white rounded-2xl font-bold shadow-lg active:scale-95">登入後台</button>
              <button onClick={()=>setView('main')} className="w-full py-2 text-gray-400 font-bold text-sm">返回首頁</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-gray-100 pb-24">
        ...
        <div className="max-w-4xl mx-auto mt-4 px-4 space-y-4">
          <div className="flex bg-white rounded-2xl shadow-sm p-1 border overflow-x-auto">
            {[{id:'stats', label:'數據統計', icon: BarChart3}, {id:'staff', label:'人員名單', icon: Users}, {id:'items', label:'規則管理', icon: ClipboardList}, {id:'history', label:'報表紀錄', icon: History}].map(tab => (
          ...
          <div className="bg-white p-5 rounded-3xl shadow-sm min-h-[500px] border border-gray-200">
            {(activeTab === 'stats' || activeTab === 'history') && (
              <div className="flex flex-col md:flex-row gap-3 mb-6 p-4 bg-gray-50 rounded-2xl border">
                ...
                <div className="flex-1"><label className="text-[10px] font-black text-gray-400 pl-1 uppercase">人員篩選</label><select value={filterStaff} onChange={e=>setFilterStaff(e.target.value)} className="w-full p-2 border-none rounded-lg font-bold bg-white mt-1 shadow-sm outline-none"><option value="all">全體彙總</option>{staffList.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              </div>
            )}
            {activeTab === 'stats' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center"><h3 className="font-bold border-l-4 border-[#c5a065] pl-2 text-gray-800">績效分析</h3>{filterStaff === 'all' ? (<button onClick={() => exportData('all')} className="bg-green-600 text-white px-3 py-2 rounded-lg text-[11px] font-bold flex items-center gap-1 shadow-sm"><Download size={14}/> 匯出總表</button>) : (<div className="flex gap-2"><button onClick={() => exportData('staff_defects')} className="bg-red-500 text-white px-3 py-2 rounded-lg text-[11px] font-bold flex items-center gap-1 shadow-sm"><ClipboardList size={14}/> 缺失表</button><button onClick={() => exportData('boss_bonus')} className="bg-green-600 text-white px-3 py-2 rounded-lg text-[11px] font-bold flex items-center gap-1 shadow-sm"><DollarSign size={14}/> 津貼表</button></div>)}</div>
                {filterStaff === 'all' ? (
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-32">
      ...
      <div className="max-w-2xl mx-auto px-4 mt-6 space-y-6">
        ...
        <div className="bg-white p-6 rounded-3xl space-y-2 border border-gray-200 shadow-sm"><label className="text-[10px] font-black text-red-400 flex items-center gap-1 pl-2 uppercase tracking-widest font-bold"><AlertTriangle size={14}/> 異常匯報或備註 (選填)</label><textarea placeholder="若有細節請在此輸入..." className="w-full p-4 bg-gray-50 border-none rounded-2xl h-24 text-sm resize-none outline-none text-gray-700" value={formData.manualNote} onChange={e=>setFormData({...formData, manualNote: e.target.value})} /></div>
      </div>

      {/* 成功彈窗 */}
      {view === 'success' && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-6 z-[200] backdrop-blur-md">
          <div className="bg-white p-10 rounded-[3rem] w-full max-w-sm text-center shadow-2xl border border-gray-100">
            <div className="text-6xl mb-6">{lastSubmitType === 'defect' ? '🚨' : '💰'}</div>
            <h2 className="text-2xl font-bold mb-2 text-gray-800 tracking-tight">上報完成！</h2>
            <div className="bg-gray-50 p-6 rounded-3xl mb-8 space-y-2 border border-gray-100">{lastSubmitType === 'defect' ? (<p className="text-red-500 font-bold text-lg tracking-tight">缺失次數：{checklistItems.filter(i => DEFECT_PAGES.includes(i.category) && formData.checkedItems[i.id]).length} 項</p>) : (<p className="text-green-600 font-black text-3xl">$ {checklistItems.filter(i => DUTY_PAGES.includes(i.category) && formData.checkedItems[i.id]).reduce((s,i)=>s+(i.value||0),0)}</p>)}<p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Saved Successfully</p></div>
